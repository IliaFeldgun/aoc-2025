use crate::{aoc_lib::expect::Number, mat_lib::loader::load_file};
...
pub fn get_day_example_result(number: i32, part: i32) -> Result<Number, String> {
    let path = format!("./inputs/{number}/{part}/example.output.expect");
    get_result(path)
}

pub fn get_my_result(number: i32, part: i32) -> Result<Number, String> {
    let path = format!("./inputs/{number}/{part}/my.output.expect");
    get_result(path)
}
fn get_result(path: String) -> Result<Number, String> {
    match load_file(&path) {
        Err(e) => Err(e),
        Ok(result) => match result.trim().parse::<i128>() {
            Err(e) => Err(format!("Failed to parse {result}: {e}")),
            Ok(number) => Ok(Number::Int128(number)),
        },
    }
}
